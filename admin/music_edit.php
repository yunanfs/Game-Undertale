    ...
    if (empty($title)) {
        ...
        if (isset($_FILES['music_file']) && $_FILES['music_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_ext, $allowed_ext)) {
                ...
                $new_file_path = 'assets/uploads/music/' . $unique_name;
        ...
        if (empty($error)) {
            $stmt = $conn->prepare("UPDATE music SET title = ?, file_path = ?, order_number = ? WHERE id = ?");
            $stmt->bind_param("ssii", $title, $file_path, $order_number, $music_id);
            
            if ($stmt->execute()) {
                // Redirect to dashboard after success
                header('Location: dashboard.php?success=Music+berhasil+diupdate');
                exit;
            } else {
                $error = 'Gagal mengupdate music: ' . $stmt->error;
            }
            $stmt->close();
        }
    }
...
<head>
    ...
    <title>Edit Music - UNDERTALE Admin</title>
...
<body>
    ...
    <div class="top-navbar">
        <div class="navbar-logo">★ EDIT TRACK ★</div>
        ...
        <div class="form-container">
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" required value="<?php echo htmlspecialchars($music['title']); ?>">
                </div>
                
                <div class="form-group">
                    <label for="music_file">AUDIO FILE (MP3, OGG, WAV)</label>
                    <?php if (!empty($music['file_path'])): ?>
                        <div style="margin-bottom: 15px; border: 1px solid #333; padding: 10px;">
                            <div style="font-size: 0.6rem; margin-bottom: 5px;">Current File: <?php echo htmlspecialchars(basename($music['file_path'])); ?></div>
                    ...
                    <div style="font-size: 0.5rem; color: #aaa; margin-top: 5px;">Leave empty to keep current file</div>
                </div>
                
                <div class="form-group">
                    <label for="order_number">ORDER NUMBER</label>
                    <input type="number" id="order_number" name="order_number" min="0" value="<?php echo $music['order_number']; ?>">
                </div>
